...
def main(cand_path, output_path, bin_output_path):
    def _(img_path):
        ...
        # get image throught z_coord observable
        ...
        save_image = z_coord.zip(
            ...
            lambda data: partial(
                utils.save_scan, data["image"], patient_id, data["z_coord"]
            )
        )

        save_image.map(lambda fn: fn(output_path)).subscribe()
        
        save_image.map(lambda fn: fn(bin_output_path, file_format="npy")).subscribe()
...
if __name__ == "__main__":
    ...
    with os.scandir(INPUT_PATH) as subset:
        ...
        ).subscribe(lambda el: print("Finished!"))
